...
For the new-routed-service checklist, see the `add-caddy-route` skill in `.claude/skills/`.
...
**When adding a new Caddy snippet (`infra/caddy/sites/*.caddy`), always update `route_prefixes` in `cli/__init__.py` in the same commit.** The CLI's `_validate_site_snippet_routes` checks every `handle` directive against `route_prefixes` during shared deploy — if a new snippet's prefix isn't listed, shared deployments abort.
