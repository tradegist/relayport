...
def _fetch_droplet_state(droplet_ip: str) -> str | None:
    ...
    Returns the captured output, or None on SSH failure. Output is bounded by
    ``--tail`` and ``--since`` so the payload size sent to claude is predictable.
    """
    cfg = config()
    remote_cmd = (
        f"cd {cfg.remote_dir} && "
        f"echo '=== docker compose ps ===' && docker compose ps && "
        f"echo && echo '=== docker compose logs --since {_LOG_SINCE} --tail {_LOG_TAIL} ===' && "
...
def run_sanity_check(droplet_ip: str) -> None:
    ...
    then feeds the captured text to a non-interactive ``claude --print`` call
    for summarization. Claude runs with no tools — pure text-in / text-out, so
    there is no permission bypass and no risk of agent-driven shell execution.
    ...
    prompt = _SUMMARIZE_PROMPT.format(droplet_output=droplet_output)
...
def post_deploy_sanity_check(droplet_ip: str, *, skip_flag: bool) -> None:
    ...
    if skip_flag or skip_post_deploy_check():
