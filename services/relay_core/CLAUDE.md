...
Mark-after-notify, atomic mark+notify boundaries, and SQLite commit discipline — see the **Reliability (MANDATORY)** section in the root [CLAUDE.md](../../CLAUDE.md). These rules are most often violated in `poller_engine.py` and `listener_engine.py`.
...
To add a new broker, see the [`add-relay-adapter`](../../.claude/skills/add-relay-adapter/SKILL.md) skill.
...
- `init_db` performs an idempotent `ALTER TABLE` migration (see PRAGMA-gated pattern in root CLAUDE.md).
