...
- **`MD_API_TOKEN`** is required at startup via `validate_api_token()`. Auth middleware rejects empty tokens with HTTP 500 (same pattern as the relay `API_TOKEN` guard — see root CLAUDE.md).
